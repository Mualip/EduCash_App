...
if (isset($_GET["keyword"])) {
    ...
    // Query untuk mengambil siswa sesuai dengan id_admin dan dengan paginasi
    $query = "SELECT * FROM siswa WHERE id_admin = ? LIMIT ?, ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("iii", $id_admin, $start, $perPage); // Bind parameter id_admin, start, perPage
    $stmt->execute();
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="lulus_siswa.php?id=<?= $row['nis_siswa']; ?>"
                                    class="btn btn-danger btn-sm mr-2">Lulus</a>
                            </td>
