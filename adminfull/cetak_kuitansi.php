...
table {
    ...
    .small-font {
        border: 3px solid #e0e0e0;
    padding: 2px; /* Ruang dalam sel agar konten tidak terlalu rapat dengan border */
    text-align: center; /* Menyusun teks ke tengah */
    font-size: 12px; /* Ukuran font untuk teks di dalam sel */
}
   /* CSS untuk mencetak */
@media print {
    ...
    @page {
        margin: none; /* Menonaktifkan margin */
    }
    ...
    .container {
        ...
        left: 0;
        width: 97%;
        border: none; /* Menghilangkan border di container */
    }
    ...
    .small-font {
    border: 3px solid #e0e0e0;
    padding: 4px 20px 4px 10px; /* Padding atas, kanan, bawah, kiri */
...
<body>
    ...
    <div class="container">
        ...
        <div style="margin-top: 5px; border-top: 3px dashed #ccc; padding-top: 10px;">
                    ...
                    <tr>
                        ...
                        <td style="width: 50%; padding-right: 10px;">
                            <div class="form-group">
                                ...
                                class="custom-select" style="width: 60%; max-width: 700px; font-size: 12px;">
...
if (!empty($namaSiswa)) {
    ...
    // Query untuk mengambil pembayaran sebelumnya yang lebih lama dari 2 menit yang lalu
    $sqlLastPayment = "SELECT total.*, siswa.nama_siswa 
        FROM total
        JOIN siswa ON total.nis_siswa = siswa.nis_siswa
        WHERE siswa.nama_siswa = '$namaSiswa' 
        AND total.tanggal_bayar < '$waktuBatas' 
        ORDER BY total.tanggal_bayar DESC"; // Urutkan berdasarkan waktu terakhir
    $resultLastPayment = $mysqli->query($sqlLastPayment);
    $lastPayments = []; // Array untuk menampung semua pembayaran sebelumnya
    while ($payment = $resultLastPayment->fetch_assoc()) {
...
if ($resultNewPayment && $resultNewPayment->num_rows > 0) { ?>
    <table>
        ...
        <tbody>
            ...
            // Menambahkan pembayaran sebelumnya (lebih dari 2 menit yang lalu) setelah pembayaran pertama
            // Menyaring pembayaran sebelumnya yang sama jenis dan hanya menampilkan pembayaran terbaru untuk tiap jenis pembayaran
            $lastJenisPembayaran = ''; // reset
            $jenisPembayaranSebelumnya = []; // Array untuk menyimpan jenis pembayaran yang sudah muncul sebelumnya

            foreach ($lastPayments as $lastPayment) {
                $biaya_pertahun = $lastPayment['total_pertahun'];
                $jumlahBayar = $lastPayment['jumlah_bayar'] ?? 0;
                $diskon = $lastPayment['diskon'] ?? 0;
                $jenisPembayaran = $lastPayment['jenis_pembayaran'] ?? 'Pembayaran Lainnya';
                $tanggalBayar = $lastPayment['tanggal_bayar'] ?? '-';
                $sisaPembayaran = $lastPayment['sisa_pembayaran'] ?? ($biaya_pertahun - $jumlahBayar - $diskon); // Ambil sisa pembayaran jika ada, jika tidak hitung manual

                $totalSisaPembayaran += $sisaPembayaran;

                // Hanya tampilkan jika jenis pembayaran belum muncul
                if (!in_array($jenisPembayaran, $jenisPembayaranSebelumnya)) {
                    // Menambahkan pembayaran sebelumnya dengan keterangan "Pbyr sbl"
                    $pembayaranData[] = [
                        'no' => $i++,
                        'jenis_pembayaran' => 'Pbyr sbl ' . formatJenisPembayaran($jenisPembayaran), // Menambahkan keterangan "Pbyr sbl"
                        'tahun_ajaran' => htmlspecialchars($lastPayment['tahun_ajaran']),
                        'total_pertahun' => number_format($biaya_pertahun, 0, ',', '.'),
                        'jumlah_bayar' => number_format($jumlahBayar, 0, ',', '.'),
                        'diskon' => number_format($diskon, 0, ',', '.'),
                        'tanggal_bayar' => htmlspecialchars($tanggalBayar),
                    ...
                    // Menambahkan jenis pembayaran ke array yang sudah ditampilkan
                    $jenisPembayaranSebelumnya[] = $jenisPembayaran;
                }
            }
            ...
            while ($pembayaran = $resultNewPayment->fetch_assoc()) {
                ...
                $pembayaranData[] = [
                    ...
                    'tahun_ajaran' => htmlspecialchars($pembayaran['tahun_ajaran']),
                    'total_pertahun' => '', // Biarkan kosong di pembayaran kedua dan seterusnya
                    'jumlah_bayar' => number_format($jumlahBayar, 0, ',', '.'),
                    'diskon' => number_format($diskon, 0, ',', '.'),
                    'tanggal_bayar' => htmlspecialchars($tanggalBayar),
                    'sisa_pembayaran' => number_format($sisaPembayaran, 0, ',', '.'),
            ...
            foreach ($pembayaranData as $data) { ?>
                <tr>
                    ...
                    <td class="small-font"><?= ($data['is_same_jenis']) ? $data['jenis_pembayaran'] : $data['jenis_pembayaran']; ?></td>
    ...
    ?>
        <tr>
            ...
            <td><?= number_format($totalBiayaPertahunFooter, 0, ',', '.'); ?></td>
            <!-- Total Jumlah Bayar: Menjumlahkan semua jumlah bayar -->
            <td><?= number_format($totalJumlahBayarFooter, 0, ',', '.'); ?></td>
            <!-- Total Diskon: Menjumlahkan semua diskon -->
            <td><?= number_format($totalDiskonFooter, 0, ',', '.'); ?></td>
            <td></td>
