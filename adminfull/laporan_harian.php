...
$tanggal = isset($_GET['tanggal']) ? $_GET['tanggal'] : ''; // Menambahkan variabel tanggal
...
// Kondisi untuk admin tertentu
if ($id_admin != 1) {
    $totalQuery .= " AND t.id_admin = '$id_admin'";  // Admin selain id_admin = 1 hanya dapat melihat data mereka sendiri
}
...
if ($tanggal) {
    $totalQuery .= " AND DATE(t.tanggal_bayar) = '$tanggal'"; // Filter berdasarkan tanggal
}
...
<form id="form-pilih" method="post">
    ...
    <?php foreach ($siswa as $row): ?>
        <tr>
            ...
            <td class="text-nowrap"><?= "Rp. " . number_format($row["jumlah_bayar"] + $row["diskon"], 0, ',', '.'); ?></td>
            <td class="text-nowrap"><?= $row["bulan"]; ?></td>
            ...
            <td class="text-nowrap"><?= $row["teller"]; ?></td>
        </tr>
        <?php $i++; ?>
    <?php endforeach; ?>
</tbody>
            ...
            <tr class="font-weight-bold bg-light">
                ...
                <td class="text-nowrap"><?= "Rp. " . number_format($totalJumlahBayar, 0, ',', '.'); ?></td>
                <td class="text-nowrap"><?= "Rp. " . number_format($totalDiskon, 0, ',', '.'); ?></td>
                <td class="text-nowrap"><?= "Rp. " . number_format($totalSetelahDiskon, 0, ',', '.'); ?></td>
                <td colspan="4"></td>
            </tr>
        </table>
