...
// Query untuk mengambil data siswa mutasi berdasarkan id_admin
if ($id_admin == 1) {
    // Admin dengan id_admin = 1 dapat melihat semua data siswa dan mutasi
    $query = "SELECT 
                nis_siswa, nama_siswa, jenis_kelamin, jenjang, kelas,
                nama_ayah, no_hp_ayah, nama_ibu, no_hp_ibu, alamat, tahun_ajaran, tanggal_mutasi
              FROM siswa_mutasi
              LIMIT $start, $perPage";
} else {
    // Admin selain id_admin = 1 hanya bisa melihat data siswa dan mutasi miliknya sendiri
    $query = "SELECT 
                nis_siswa, nama_siswa, jenis_kelamin, jenjang, kelas,
                nama_ayah, no_hp_ayah, nama_ibu, no_hp_ibu, alamat, tahun_ajaran, tanggal_mutasi
              FROM siswa_mutasi
              WHERE id_admin = '$id_admin'  -- Menambahkan filter berdasarkan id_admin
              LIMIT $start, $perPage";
}
...
if ($id_admin == 1) {
    ...
    $totalQuery = "SELECT COUNT(*) FROM siswa_mutasi";
} else {
    // Admin selain id_admin = 1 hanya bisa melihat data siswa dan mutasi miliknya sendiri
    $totalQuery = "SELECT COUNT(*) FROM siswa_mutasi WHERE id_admin = '$id_admin'";
}
...
<div class="container-fluid mt-4">
    <h6 class="font-weight-bold text-primary">Daftar Siswa Keluar</h6>
            ...
            <tbody >
                ...
                <?php foreach ($siswa as $row): ?>
                    <tr>
                        ...
                        <td class="text-nowrap"><?= date('d-m-Y', strtotime($row["tanggal_mutasi"])); ?></td> <!-- Format tanggal -->
                    </tr>
                    <?php $i++; ?>
                <?php endforeach; ?>
            </tbody>
