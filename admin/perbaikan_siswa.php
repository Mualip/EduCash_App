...
// Ambil id_admin yang terautentikasi dari session
$id_admin = $_SESSION['id_admin'];

// Ambil nilai pencarian jika ada, atau default ke string kosong
$searchTerm = isset($_GET['search']) ? $_GET['search'] : '';

// Query untuk mengambil data siswa berdasarkan id_admin dan pencarian
$query = "SELECT * FROM siswa WHERE id_admin = ?";
if ($searchTerm) {
    $query .= " AND (nis_siswa LIKE ? OR nama_siswa LIKE ? OR keaktifan LIKE ?)";
}

// Menjalankan query dengan prepared statement untuk mencegah SQL Injection
$stmt = $mysqli->prepare($query);

// Jika ada pencarian, kita bind parameter pencarian
if ($searchTerm) {
    $searchTermParam = "%" . $searchTerm . "%";
    $stmt->bind_param("ssss", $id_admin, $searchTermParam, $searchTermParam, $searchTermParam);
} else {
