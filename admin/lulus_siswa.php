...
session_start();  // Mulai session di bagian atas file
...
// Proses lulus data siswa jika form disubmit
if (isset($_POST['lulus_btn'])) {
    // Ambil data dari form
    $tanggal_lulus = $_POST['tanggal_lulus'];

    // Persiapkan query untuk memasukkan data ke tabel siswa_lulus
    $insert_lulus_query = "INSERT INTO siswa_lulus 
                            (nis_siswa, nama_siswa, jenis_kelamin, jenjang, kelas, nama_ayah, no_hp_ayah, nama_ibu, no_hp_ibu, alamat, tahun_ajaran, tanggal_lulus, id_admin) 
                            VALUES 
                            ('{$data['nis_siswa']}', '{$data['nama_siswa']}', '{$data['jenis_kelamin']}', '{$data['jenjang']}', '{$data['kelas']}', 
                            '{$data['nama_ayah']}', '{$data['no_hp_ayah']}', '{$data['nama_ibu']}', '{$data['no_hp_ibu']}', '{$data['alamat']}', 
                            '{$data['tahun_ajaran']}', '$tanggal_lulus', '$id_admin')";

    // Eksekusi query untuk menyimpan data ke tabel siswa_lulus
    if (mysqli_query($mysqli, $insert_lulus_query)) {
        // Setelah berhasil menyimpan data lulus, update kolom keaktifan di tabel siswa
        $update_siswa_query = "UPDATE siswa SET keaktifan = 'LULUS' WHERE nis_siswa = '$nis_siswa'";

        if (mysqli_query($mysqli, $update_siswa_query)) {
            echo "<script>alert('Data berhasil diluluskan dan status keaktifan diperbarui!'); window.location='daftarsiswa.php';</script>";
        } else {
...
<li class="nav-item active">
    ...
    <div id="collapseEditIdentitas" class="collapse" aria-labelledby="headingEditIdentitas" data-parent="#accordionSidebar">
        ...
        <form action="" method="post">
            ...
            <div class="mb-3">
                <label for="tanggal_lulus" class="form-label">Tanggal Kelulusan</label>
                <input type="date" class="form-control" id="tanggal_lulus" name="tanggal_lulus" required>
            </div>
            <button type="submit" name="lulus_btn" class="btn btn-primary">Lulus</button>
        </form>
    </div>
